...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:Ago0#1!$@127.0.0.1/e_commerce_db'
...
orders_get_schema = OrderGetSchema(many=True)
...
def add_customer():
    ...
    db.session.commit()
    return jsonify({"message": "New customer added successfully", "id": new_customer.id}), 201
...
def get_orders():
    ...
    return orders_get_schema.jsonify(orders)
